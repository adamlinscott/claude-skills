 ...
 *
 * CLAUDE_SKILLS_FORCE_PROMPT overrides the TTY check. Everything below this line was already
 * built to be driven by piped keystrokes — see the header comment, and the `pending` queue that
 * exists precisely because a pipe delivers the whole script in one burst — but the TTY gate meant
 * no test could ever reach it. The env var opens that door and is read nowhere else.
 */
export function canPrompt() {
  if (process.env.CLAUDE_SKILLS_FORCE_PROMPT === "1") return true;
