 ...
 *
 * A word with no break in it — a URL, a long path — is cut to fit rather than left to run past
 * the edge. The details panel draws a border around whatever comes back, so one over-long line
 * there does not just look untidy, it pushes the border out and misaligns every row beside it.
 */
export function wrap(text, width) {
  ...
  for (const word of String(text).split(/\s+/).filter(Boolean)) {
    ...
    if (!rest) continue;
    if (line && `${line} ${rest}`.length > limit) {
      lines.push(line);
      line = rest;
    } else {
...
// The details panel is readable between these two widths. Below the first it is not worth the
// space it takes; above the second the prose gets too wide to scan.
...
/**
 * Lay the list on the left and its details panel on the right, or stack them when the caller has
 * worked out that they will not fit side by side.
 */
function twoColumn(listLines, boxLines, stacked) {
  if (stacked) return [...listLines.map((l) => `  ${l}`), "", ...boxLines.map((l) => `  ${l}`)];
...
export async function multiSelect({ heading, note, items, selected = [], confirmLabel = "Confirm selection" }) {
  ...
  const paint = () => {
    ...
    // Measured from the list actually on screen, not guessed: the widest row decides whether the
    // panel fits beside it at all, and how much width is left for it if it does. A fixed guess
    // put a 33-column panel next to a 52-column list on an 80-column terminal, and every row of
    // the pair wrapped onto the next line.
    const gutter = 2;
    const gap = 3;
    const room = termWidth() - gutter - Math.max(...list.map(visibleWidth)) - gap;
    const stacked = room < MIN_BOX_WIDTH;
    const boxWidth = stacked
      ? Math.max(MIN_BOX_WIDTH, Math.min(MAX_BOX_WIDTH, termWidth() - gutter * 2))
      : Math.min(MAX_BOX_WIDTH, room);
    ...
    out.push("");
    out.push(...twoColumn(list, box, stacked));
