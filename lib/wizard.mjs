 ...
 */
function detailBox(title, body, width, maxBodyLines = 9) {
  const inner = width - 4;
  const bar = "─".repeat(width - 2);
  const row = (text) => `│ ${text.padEnd(inner)} │`;
  const lines = [`┌${bar}┐`, `│ ${style.bold(String(title).slice(0, inner).padEnd(inner))} │`];
  const body_ = wrap(body || "", inner).slice(0, maxBodyLines);
  if (body) {
    lines.push(row(""));
    for (const l of body_) lines.push(style.dim(row(l)));
  }
  lines.push(`└${bar}┘`);
  return lines;
}
...
export async function multiSelect({ heading, note, items, selected = [], confirmLabel = "Confirm selection" }) {
  ...
  const paint = () => {
    ...
    if (note) out.push(`  ${style.dim(note)}`);
    out.push("");
